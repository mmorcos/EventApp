...
$(document).ready(function() {
    ...
    openRequest.onupgradeneeded = function(e) {
        ...
        if(!thisDb.objectStoreNames.contains("note")) {
			...
			objectStore.createIndex("titlel1", "titlel1", { unique: false });
    ...
    function displayNotes(filter) {
        ...
        var content="<table class='table table-bordered table-striped'><thead><tr><th>Events Details </th><th>Updated</th><th>&nbsp;</td></thead><tbody>";
    ...
    $("#saveNoteButton").on("click",function() {
        ...
        if(key === "") {
            t.objectStore("note")
                            .add({title:title,body:body,updated:new Date(),titlelc:titlelc,tags:tags,title1:title1});
        } else {
            t.objectStore("note")
                            .put({title:title,body:body,updated:new Date(),id:Number(key),titlelc:titlelc,tags:tags,title1:title1});
        }
		...
		var handleResult = function(event) {
			...
			if(cursor) {
				if(cursor.value.id != parentNote) {
					doneOne = true;
					content += "<a class='loadNote' data-noteid='"+cursor.value.id+"'>" + cursor.value.title + "</a><br/>  " + cursor.value.title1;
				}
				cursor.continue();
			}			
		};
